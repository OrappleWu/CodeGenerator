 ...
 */
public class ServiceCodeTemplate extends DaoCodeTemplate {

    private String rootFileCatalog;

    public ServiceCodeTemplate(InputStream inputStream, String rootFileCatalog) {
    ...
    public FileWriter initFileWriter() {
        String fileName = "I".concat(CodeTemplateContext.getVal(AbstractCodeTemplate.ENTITY_NAME)).concat("Service.java");
        String filePath = this.rootFileCatalog.concat("/main/java/") +
                this.transferPageNameToPath(CodeTemplateContext.getVal(AbstractCodeTemplate.BASE_PACKAGE)) + "/service";
