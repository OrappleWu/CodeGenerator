 ...
 */
public class EntityCodeTemplate extends DaoCodeTemplate {
    private String rootFileCatalog;

    public EntityCodeTemplate(InputStream inputStream, String rootFileCatalog) {
    ...
    public FileWriter initFileWriter() {
        String fileName = CodeTemplateContext.getVal(AbstractCodeTemplate.ENTITY_NAME) + ".java";

        String filePath = this.rootFileCatalog.concat("/main/java/") +
                this.transferPageNameToPath(CodeTemplateContext.getVal(AbstractCodeTemplate.BASE_PACKAGE)) + "/entity";
